...
QuincePuntos::QuincePuntos() :CampoAbs(9, 5)
{
	nombreMatriz = "Quince Puntos";
}
...
bool QuincePuntos::agregar(Coordenada* x1y1, Coordenada* x2y2,int p)//2-0
{
...
bool QuincePuntos::vacio()
{
	return false;
}

bool QuincePuntos::unirCamposJuego(CampoAbs*)
{
	return false;
}

bool QuincePuntos::asignarNumeroXCuadro(int num)
{
	int contador = 0;
	for (int i = 0; i < filas; i++) {
		for (int j = 0; j < columnas; j++) {
...
void QuincePuntos::pintarMatriz(int m)
{
	CampoAbs::pintarMatriz(m);
}
